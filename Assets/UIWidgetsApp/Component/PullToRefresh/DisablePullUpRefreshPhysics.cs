...
{
    public class DisablePullUpRefreshScrollPhysics : ScrollPhysics
    {
        public DisablePullUpRefreshScrollPhysics(
        ...
        private static float frictionFactor(float overScrollFraction)
        {
        ...
        {
            return new DisablePullUpRefreshScrollPhysics(buildParent(ancestor: ancestor),
        ...
        private static float _applyFriction(float extentOutside, float absDelta, float gamma)
        {
            D.assert(absDelta > 0);
            var total = 0.0f;
            if (extentOutside > 0)
